...
routerAdd(
  ...
  (e) => {
    ...
    return e.json(200, { url: base + "/api/ghostlight/cal/" + token + "/calls.ics" });
  },
  $apis.requireAuth(),
);

routerAdd("GET", "/api/ghostlight/cal/{token}/calls.ics", (e) => {
  ...
  for (const m of memberships) {
    ...
    for (const ev of events) {
      ...
      lines.push("UID:evt-" + ev.id + "@ghostlight");
