...
//
// Ghostlight server hooks: passwordless signup, join-by-code, default
...
routerAdd("POST", "/api/ghostlight/signup", (e) => {
...
// Ghostlight is app-only.

function ghostlightRecipients(app, productionId, calledMemberIds) {
...
function ghostlightSendMail(app, recipients, subject, html) {
...
onRecordAfterCreateSuccess((e) => {
  try {
    ...
    const recipients = ghostlightRecipients(e.app, production.id, e.record.get("called"));
    ...
    const lines = [
      ...
      `<p>Open Ghostlight to tap "Got it" so your stage manager knows you saw this.</p>`,
    ];
    ghostlightSendMail(
      e.app,
      recipients,
    ...
    e.app.logger().error("ghostlight: event mail failed", "error", String(err));
  }
  e.next();
}, "events");
...
onRecordAfterCreateSuccess((e) => {
  try {
    const production = e.app.findRecordById("productions", e.record.get("production"));
    const recipients = ghostlightRecipients(e.app, production.id, null);
    ghostlightSendMail(
      e.app,
      recipients,
      `[${production.get("title")}] ${e.record.get("title")}`,
      `<h2>${e.record.get("title")}</h2><p>${e.record.get("body") || ""}</p><p>— ${production.get("title")} on Ghostlight</p>`,
    );
  } catch (err) {
    e.app.logger().error("ghostlight: announcement mail failed", "error", String(err));
  }
  e.next();
}, "announcements");
