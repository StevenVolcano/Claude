...
function smsConfigured() {
  const p = $os.getenv("GHOSTLIGHT_SMS_PROVIDER");
  return p === "twilio" || p === "telnyx";
}

function sendSmsReal(app, to, body) {
  const provider = $os.getenv("GHOSTLIGHT_SMS_PROVIDER");
  if (!smsConfigured()) {
    app.logger().info("ghostlight sms (dormant, not sent)", "to", to, "body", body);
    return false;
  }
  try {
    if (provider === "twilio") {
      ...
      if (res.statusCode >= 300) {
        app.logger().error("ghostlight sms: twilio error", "status", res.statusCode, "body", res.raw);
        return false;
      }
      return true;
    }
    if (provider === "telnyx") {
      ...
      if (res.statusCode >= 300) {
        app.logger().error("ghostlight sms: telnyx error", "status", res.statusCode, "body", res.raw);
        return false;
      }
      return true;
    }
  } catch (err) {
    app.logger().error("ghostlight sms: send failed", "error", String(err));
  }
  return false;
}
...
function createAndSendCode(app, phone, purpose, userId, ip) {
  ...
  rec.set("attempts", 0);
  app.save(rec);
  sendSmsReal(app, phone, "Ghostlight code: " + code + ". It expires in 10 minutes.");
}
...
routerAdd("POST", "/api/ghostlight/signin-sms/start", (e) => {
  const data = new DynamicModel({ phone: "" });
  e.bindBody(data);
  const phone = normalizeUsPhone(data.phone);
  if (!phone) throw new BadRequestError("Enter a 10-digit US phone number.");
...
routerAdd("POST", "/api/ghostlight/signin-sms/confirm", (e) => {
  const data = new DynamicModel({ phone: "", code: "" });
  e.bindBody(data);
  const phone = normalizeUsPhone(data.phone);
  if (!phone) throw new BadRequestError("Enter a 10-digit US phone number.");
